...
class MultiHeadAttentionBlock(nn.Module):
    ...
    def forward(self, q, k, v, mask):
        ...
        key = self.w_k(k)
        value = self.w_v(v)
        # {batch, seq_len,d_model} ->  (batch,seq_len,h,d_k) --> (batch,h,seq_len,k_)
...
class EncoderBlock(nn.Module):
    def __init__(
        ...
        self.residual_connections = nn.ModuleList(
            [ResidualConnectionBlock(dropout) for _ in range(2)]
        )
...
class DecoderBlock(nn.Module):
    ...
    def __init__(
        ...
        self.residual_connections = nn.Module(
            [ResidualConnectionBlock(dropout) for _ in range(3)]
        )
...
def build_transformer(
    ...
    # create encoder and the decoder
